analyzeCutblockFireOverlap <- function(cutLays_with_fire, cutLays_no_fire, fireLays) {
  ...
  # Each fire layer need to have one specific years and be separated by runs in a list
  # Note: Forestry when fire is present, ignores fires for that SPECIFIC year, but not
  # from previous years. Ex. if a pixel burns in 2021, in 2021 there will be no 
  # forestry there. However, there might be in 2031 (i.e., 10 year old forest, 
  # but if potential was good, it might be cut as a 50 year old forest). Should 
  # eventually be fixed so that fire layers update potential. But may not be 
  # desired because it is a simulated fire, not a real one... 
  
    # names() <- paste0("run0", 1:5)

  ################ HERE BELOW -- THEY SEEM IDENTICAL! CHECK AND FIX!
  source("R/separateByReplicate.R")
  cutFires <- separateByReplicate(spatVector = cutLays_with_fire)
  cutNoFires <- separateByReplicate(spatVector = cutLays_no_fire)
  
  # --- 3. Perform Overlap Checks ---
  
  allRuns <- lapply(names(fireLays), function(run){
    cutFire <- cutFires[[run]]
    cutNoFire <- cutNoFires[[run]]
    fireLay <- fireLays[[run]]
    eachYear <- lapply(unique(cutFire$DisturbanceYear), function(YR){
      yr <- paste0("Year", YR)
      fireLayYear <- fireLay[[yr]]
      cutFireYear <- cutFire[cutFire$DisturbanceYear == YR]
      cutNoFireYear <- cutNoFire[cutNoFire$DisturbanceYear == YR]
      cutFireOverlap <- terra::extract(fireLayYear, cutFireYear, 
                                                    ID=TRUE, fun=mean, na.rm=TRUE)
      cutNoFireOverlap <- terra::extract(fireLayYear, cutNoFireYear, 
                                       ID=TRUE, fun=mean, na.rm=TRUE)
      browser() # WTF?!
    }) 
